...
# MAGIC 
# MAGIC 
# MAGIC [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/JohnSnowLabs/spark-nlp-workshop/blob/master/tutorials/Certification_Trainings/Public/5.Text_Classification_with_ClassifierDL.ipynb)

# COMMAND ----------

# MAGIC %md
# MAGIC # 5 Text Classification with ClassifierDL
...
# MAGIC %md
# MAGIC 
# MAGIC <b>  if you want to work with Spark 2.3 </b>
# MAGIC ```
# MAGIC import os
# MAGIC 
# MAGIC # Install java
# MAGIC ! apt-get update -qq
# MAGIC ! apt-get install -y openjdk-8-jdk-headless -qq > /dev/null
# MAGIC 
# MAGIC !wget -q https://archive.apache.org/dist/spark/spark-2.3.0/spark-2.3.0-bin-hadoop2.7.tgz
# MAGIC 
# MAGIC !tar xf spark-2.3.0-bin-hadoop2.7.tgz
# MAGIC !pip install -q findspark
# MAGIC 
# MAGIC os.environ["JAVA_HOME"] = "/usr/lib/jvm/java-8-openjdk-amd64"
# MAGIC os.environ["PATH"] = os.environ["JAVA_HOME"] + "/bin:" + os.environ["PATH"]
# MAGIC os.environ["SPARK_HOME"] = "/content/spark-2.3.0-bin-hadoop2.7"
# MAGIC ! java -version
# MAGIC 
...
# MAGIC 
# MAGIC ! pip install --ignore-installed -q spark-nlp==2.7.5
# MAGIC 
# MAGIC import sparknlp
# MAGIC 
# MAGIC spark = sparknlp.start(spark23=True)
# MAGIC ```
...
# MAGIC %md
# MAGIC <b><h1><font color='darkred'>!!! WARNING !!! </font><h1><b>
# MAGIC 
# MAGIC 
# MAGIC 
# MAGIC **If you get an error related to Java port not found 55, it is probably because that the Colab memory cannot handle the model and the Spark session died. In that case, try on a larger machine or restart the kernel at the top and then come back here and rerun.**
...
data = pd.read_csv("/content/drive/MyDrive/colab/datasets/reviews.csv", low_memory=False)
data.head()
...
train_X, test_X, train_y, test_y = train_test_split(data_sampled['review_body'], data_sampled['star_rating'],
                                                    stratify=data_sampled['star_rating'], 
                                                    test_size=0.25)
...
preds.select('star_rating','review_body',"class.result").show(10, truncate=80)

# COMMAND ----------

preds_df = preds.select('star_rating','review_body',"class.result").toPandas()
...
preds_df['result'] = preds_df['result'].apply(lambda x : x[0])
preds_df['star_rating'] =preds_df['star_rating'].astype(int)
preds_df['result'] =preds_df['result'].astype(int)
print (classification_report(preds_df['result'], preds_df['star_rating']))
...
# MAGIC %%time 
# MAGIC use_pipelineModel = use_clf_pipeline.fit(trainDataset)
...
# MAGIC %md
# MAGIC ### with BertSentenceEmbeddings

# COMMAND ----------

# MAGIC %md
# MAGIC #### `sent_small_bert_L8_512` Model
...
# MAGIC %%time
# MAGIC bert_sent_pipelineModel = bert_sent_clf_pipeline.fit(trainDataset)
...
# MAGIC %md
# MAGIC #### `sent_bert_use_cmlm_en_base` Model
...
# MAGIC %%time
# MAGIC bert_cmlm_pipelineModel = bert_cmlm_clf_pipeline.fit(trainDataset)
...
text='''
the screen protectors were pretty cool but they messed up my order the first time they send them to me other than that they are cool
'''
...
light_model.annotate('the screen protectors were pretty cool but they messed up my order the first time they send them to me other than that they are cool')
...
# MAGIC %%time
# MAGIC pipelineModel = pipeline.fit(trainDataset)
...
# MAGIC %md
# MAGIC ## Multilabel Classifier with Bert Embeddings
...
# MAGIC %%time
# MAGIC 
# MAGIC use_pipelineModel = use_clf_pipeline.fit(trainingData)
# MAGIC 
# MAGIC # 20 epochs takes around 20 seconds !
...
# MAGIC %%time
# MAGIC #20 epochs 30 secs !
# MAGIC 
# MAGIC bert_pipelineModel = bert_clf_pipeline.fit(trainingData)
...
# MAGIC %md
# MAGIC # Save model and Zip it for Modelshub Upload/Downloads
